...
def get_acao(acao_nome):
	...
	retorno.append("")
	retorno[-1] += f"Nome;{infos['longName']}\n"
	retorno[-1] += f"Sigla;{infos['symbol'].replace('.SA','')}\n"
	retorno[-1] += f"Industria;{infos['industry']}\n"
	retorno[-1] += f"Site;{infos['website']}\n"
	retorno[-1] += f"Valor de Mercado;R$ {infos['marketCap']:,.2f}\n"
	retorno[-1] += f"Valor da Empresa;R$ {infos['enterpriseValue']:,.2f}\n"
	retorno[-1] += f"Receita Liquida;R$ {infos['totalRevenue']:,.2f}\n"
	retorno[-1] += f"Lucro Liquido;R$ {infos['netIncomeToCommon']:,.2f}\n"
	retorno[-1] += f"Divida Bruta;R$ {infos['totalDebt']:,.2f}\n"
	retorno[-1] += f"Divida Liquida;R$ {infos['totalDebt'] - infos['totalCash']:,.2f}\n"
	retorno[-1] += f"Ebitda;R$ {infos['ebitda']:,.2f}\n"
	retorno[-1] += f"Margem Ebitda;{infos['ebitdaMargins'] * 100:.2f}%\n"
	retorno[-1] += f"P/L;{infos['trailingPE']:.2f}\n"
	retorno[-1] += f"Dividend Yield;{infos['dividendYield'] * 100:.2f}%\n"
	retorno[-1] += f"Acoes em Ciculacao;{infos['sharesOutstanding']:,.2f}\n"
